...
func (m *UserMutation) SetKakaoSub(s string) {
	m.kakao_sub = &s
}

// KakaoSub returns the value of the "kakao_sub" field in the mutation.
func (m *UserMutation) KakaoSub() (r string, exists bool) {
	v := m.kakao_sub
	if v == nil {
		return
	}
...
// OldKakaoSub returns the old "kakao_sub" field's value of the User entity.
// If the User object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *UserMutation) OldKakaoSub(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldKakaoSub is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldKakaoSub requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldKakaoSub: %w", err)
	}
	return oldValue.KakaoSub, nil
}

// ResetKakaoSub resets all changes to the "kakao_sub" field.
func (m *UserMutation) ResetKakaoSub() {
	m.kakao_sub = nil
}
...
func (m *UserMutation) Fields() []string {
	...
	if m.kakao_sub != nil {
		fields = append(fields, user.FieldKakaoSub)
	}
	if m.name != nil {
